...
// ==========================================
// 5. GESTIÓN DEL CARRITO, PEDIDOS WHATSAPP Y UI
// ==========================================
...
function vaciarCarritoYLimpiarUI() {
    vaciarCarrito();
}
...
function sendWhatsApp() {
    ...
    activeCart.forEach(item => {
        let sub = (item.price || item.precio || 0) * (item.qty || item.quantity || 1);
        total += sub;
        message += `- ${item.qty || item.quantity || 1}x ${item.name || item.nombre || 'Producto'} ($${sub.toFixed(2)})%0A`;
    });

    message += `%0A💰 *Total a Pagar:* $${total.toFixed(2)}`;

    const phone = "584127305103"; 
...
function getActiveCartItems() {
    ...
    try {
        if (typeof cart !== 'undefined' && Array.isArray(cart) && cart.length > 0) {
            ...
            const stored = localStorage.getItem('catalog_cart') || localStorage.getItem('cart') || localStorage.getItem('carrito');
...
function updateCartUI() {
    ...
    if (badge) {
        const totalItems = currentCart.reduce((sum, item) => sum + (item.qty || item.quantity || 0), 0);
...
function renderCartItems() {
    ...
    if (currentCart.length === 0) {
        container.innerHTML = `
            <div class="text-center py-12 px-4 space-y-4">
                <div class="w-20 h-20 bg-orange-100 text-orange-500 rounded-full flex items-center justify-center mx-auto text-3xl shadow-inner animate-pulse">
                    <i class="fa-solid fa-basket-shopping"></i>
                </div>
                <div>
                    <h4 class="font-bold text-gray-800 text-lg">Tu carrito está esperando</h4>
                    <p class="text-xs text-gray-500 mt-1 max-w-xs mx-auto">¡Aún no has agregado ninguna delicia! Explora nuestros productos y arma tu pedido al mayor.</p>
                </div>
                <button onclick="toggleCart()" class="bg-orange-500 hover:bg-orange-600 text-white font-semibold text-xs px-5 py-2.5 rounded-xl transition shadow-md inline-flex items-center space-x-2">
    ...
    currentCart.forEach((item, index) => {
        let price = Number(item.price || item.precio || 0);
        ...
        if (typeof products !== 'undefined' && Array.isArray(products)) {
            ...
            if (product && product.images && product.images.length > 0) {
                ...
                imgUrl = (typeof first === 'object' && first !== null) ? (first.url || '') : first;
                if (typeof getImagePositionString === 'function') {
                    imgPos = getImagePositionString(first);
                }
            }
        }
        ...
        let imageElement = '';
        if (imgUrl) {
            imageElement = `<img src="${imgUrl}" alt="${item.name || item.nombre}" class="w-14 h-14 object-cover rounded-xl border border-orange-200 shadow-2xs shrink-0" style="object-position: ${imgPos};">`;
        } else {
            imageElement = `
                <div class="w-14 h-14 bg-orange-50 border border-orange-200 rounded-xl flex items-center justify-center text-orange-400 shrink-0 shadow-2xs">
        ...
        html += `
            <div class="bg-white border border-orange-100 hover:border-orange-300 p-3 rounded-2xl flex items-center justify-between shadow-2xs transition-all duration-200 group">
                <div class="flex items-center space-x-3 flex-grow pr-2">
                    ${imageElement}
                    <div class="min-w-0 flex-grow">
                        <h4 class="font-bold text-gray-800 text-xs sm:text-sm truncate group-hover:text-orange-600 transition">${item.name || item.nombre}</h4>
                        <p class="text-[11px] text-orange-600 font-extrabold mt-0.5">$${price.toFixed(2)} <span class="text-gray-400 font-normal">c/u</span></p>
                        
                        <div class="flex items-center justify-between mt-2">
                            <div class="flex items-center border border-orange-200 rounded-xl bg-orange-50/50 overflow-hidden shadow-2xs">
                                <button type="button" onclick="changeCartQty(${index}, -1)" class="px-2.5 py-1 text-orange-600 hover:bg-orange-200 font-extrabold transition text-xs">-</button>
                                <span class="px-3 text-xs font-bold text-gray-800">${qty}</span>
                                <button type="button" onclick="changeCartQty(${index}, 1)" class="px-2.5 py-1 text-orange-600 hover:bg-orange-200 font-extrabold transition text-xs">+</button>
                            </div>
                            <span class="font-black text-gray-800 text-sm">$${subtotal.toFixed(2)}</span>
                        </div>
                    </div>
                </div>
                <button type="button" onclick="removeFromCart(${index})" class="text-gray-300 hover:text-red-500 p-2 transition ml-1 shrink-0" title="Eliminar producto">
    ...
    html += `
        <div class="mt-4 bg-gradient-to-r from-orange-500 to-amber-500 text-white p-3 rounded-2xl shadow-sm flex items-center justify-between text-xs font-medium">
            <div class="flex items-center space-x-2.5">
                <i class="fa-solid fa-basket-shopping text-base text-amber-200 animate-bounce"></i>
                <span>¡Excelente selección! ¿Quieres agregar más cajas o dulces surtidos?</span>
            </div>
            <button onclick="toggleCart()" class="bg-white text-orange-600 hover:bg-orange-50 px-3 py-1.5 rounded-xl font-bold shadow-xs transition shrink-0 ml-2">
                + Ver más
            </button>
        </div>
    `;
...
function changeCartQty(index, delta) {
    if (typeof cart !== 'undefined' && Array.isArray(cart) && cart[index]) {
        cart[index].qty = (cart[index].qty || cart[index].quantity || 1) + delta;
        if (cart[index].qty <= 0) {
            cart.splice(index, 1);
        }
        localStorage.setItem('catalog_cart', JSON.stringify(cart));
...
function addModalToCart() {
    if (typeof currentModalProduct !== 'undefined' && currentModalProduct && typeof addCurrentProductToCart === 'function') {
        addCurrentProductToCart(currentModalProduct.id);
    }
}